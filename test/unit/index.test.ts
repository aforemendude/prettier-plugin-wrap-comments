...
import plugin, { name, parsers, printers } from '../../src/index.js';

describe('plugin entry point', () => {
  it('exports a stable name and the initialized parsers and printers as one plugin', () => {
    expect(mocks.createParsers).toHaveBeenCalledTimes(1);
    expect(mocks.createPrinters).toHaveBeenCalledTimes(1);
    expect(name).toBe('@aforemendude/prettier-plugin-wrap-comments');
    expect(parsers).toBe(mocks.parsers);
    expect(printers).toBe(mocks.printers);
    expect(plugin).toStrictEqual({ name, parsers: mocks.parsers, printers: mocks.printers });
  });
});
